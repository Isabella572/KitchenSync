...
def shoppinglist_page():
    ...
    with st.form("manual_add_form"):
        ...
        if st.form_submit_button("Add to List"):
            if new_name.strip() == "":
                ...
                db_utils.add_to_shoppinglist(new_name.strip().lower(), new_quantity, new_unit.strip().lower())
                st.rerun()
    ...
    if not shopping_items:
        ...
        for item in shopping_items:
            ...
            with st.expander(f"✏️ {name.capitalize()} — {round(quantity, 2)} {unit}"):
                col1, col2, col3 = st.columns(3)
                with col1:
                    edited_name = st.text_input("Name", value=name, key=f"edit_name_{item_id}")
                with col2:
                    edited_quantity = st.number_input("Quantity", value=float(quantity), min_value=0.0, step=0.1, key=f"edit_qty_{item_id}")
                with col3:
                ...
                with col4:
                    if st.button("Save Changes", key=f"save_{item_id}"):
                        db_utils.edit_shopping_item(item_id, edited_name.strip().lower(), edited_quantity, edited_unit.strip().lower())
                        st.rerun()
                with col5:
            ...
            # bought checkbox outside the expander
            bought = st.checkbox(f"✅ Bought {name.capitalize()}", key=f"bought_{item_id}")

            if bought:
                st.write(f"How much {name} did you buy?")
                col1, col2 = st.columns(2)
                with col1:
                    bought_quantity = st.number_input(
                        "Quantity bought",
                        min_value=0.0,
                        step=0.1,
                ...
                with col3:
                    bought_expiry = st.date_input(
                        "Expiry Date",
                        key=f"bought_expiry_{item_id}"
                    )
                with col4:
                    bought_category = st.selectbox(
                        "Category",
